...
fleetSizeVar.forEach((e,i)=>{
    st2+=`<option value="${e.id}" class="SelectOption">${e.name}</option>`
})
...
const servicePop = (f)=>{
    ...
    serviceTags.forEach((e,i)=>{
        st3+=`<div class="main">
        <div class="CheckBox2">
            <input value="${e.id}" class="CheckBoxInput2" type="radio" name="ServiceTag" id="service${i}" required>
        </div>
...
const SubmitHandler1 = (e)=>{
    ...
    let ServiceTag = document.querySelector('input[name=ServiceTag]:checked').value;
